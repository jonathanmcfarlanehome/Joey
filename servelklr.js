...
/*
 * Crowley - Modern Project Management System
...
console.log('🚀 Crowley - Modern Project Management System');
  ...
  try {
    ...
    if (!pathname.startsWith('/api') && method === 'GET') {
      ...
      if (!fs.existsSync(resolved)) {
        ...
        if (fs.existsSync(rootPath) && !rootPath.includes('..')) {
    ...
    if (method === 'PUT' && pathname.startsWith('/api/projects/')) {
      ...
      if (lead_id !== undefined) project.leadId = lead_id;
    ...
    if (method === 'GET' && pathname.startsWith('/api/projects/') && pathname.endsWith('/board')) {
      ...
      const filteredWithAttachments = filtered.map(issue => ({
        ...issue,
        attachments: attachments.filter(a => a.issueId === issue.id)
      }));
      ...
      for (const status of workflow.statuses) {
        board[status] = filteredWithAttachments.filter(i => i.status === status);
      }
...
server.listen(CONFIG.PORT, () => {
  ...
  console.log(`📂 Visit http://localhost:${CONFIG.PORT} to access Crowley`);
  if (!formidable) {
